# -*- coding: utf-8 -*-
# @Time    : 2020/7/7 上午11:18
# @Author  : chaos
# @FileName: oop_constructorMode_rewrite.py
# @Software: PyCharm
...
class Director(object, metaclass=ABCMeta):
    ...
    def get_construced_object(self):
    ...
    def construct(self, field_list):
        for field in field_list:
            if field['field_type'] == 'text_field':
                self.builder.add_text_field(field)
            elif field['field_type'] == 'checkbox':
                self.builder.add_checkbox(field)
            elif field['field_type'] == 'button':
                self.builder.add_button(field)
...
if __name__ == "__main__":
    ...
    with open('form_file_based_on_constructorMode.html', "w") as file:
        file.write(
            "<html><body>{}</body></html>".format(
                director.get_construced_object()
            )
        )